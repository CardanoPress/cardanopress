...
use CardanoPress\Dependencies\GuzzleHttp\Promise\FulfilledPromise;
use CardanoPress\Dependencies\GuzzleHttp\Promise\PromiseInterface;
use CardanoPress\Dependencies\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \CardanoPress\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \CardanoPress\Dependencies\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \CardanoPress\Dependencies\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \CardanoPress\Dependencies\GuzzleHttp\Psr7\Utils::streamFor($sink);
