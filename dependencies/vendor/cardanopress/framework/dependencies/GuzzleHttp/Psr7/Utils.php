...
final class Utils
{
    ...
    private static function warnOnInvalidModifyRequestChanges(array $changes): void
    {
        ...
        if (\array_key_exists('body', $changes) && $changes['body'] === null) {
            self::warnOnInvalidModifyRequestChange('body', 'resource|string|int|float|bool|StreamInterface|callable|\Iterator|\CardanoPress_Dependency_CardanoPress_Dependency_Stringable', $changes['body']);
        }
     ...
     * - `CardanoPress\Dependencies\Psr\Http\Message\StreamInterface`: Returns the value as-is.
        ...
        if (is_scalar($resource)) {
            if (!is_string($resource)) {
                \trigger_deprecation(
                    'guzzlehttp/psr7',
                    '2.12',
                    'Passing %s to Utils::streamFor() is deprecated; cast it to a string. guzzlehttp/psr7 3.0 will only accept string, resource, StreamInterface, CardanoPress_Dependency_CardanoPress_Dependency_Stringable, Iterator, callable, or null.',
                    \gettype($resource)
                );
