...
const CoinSelection = {
    ...
    randomImprove: async (inputs, outputs, limit) => {
        ...
        let utxoSelection = {
            ...
            subset: [],
            amount: CSL.Value.new(CSL.BigNum.from_str('0')),
        }
        ...
        for (let i = 0; i < splitOutputsAmounts.length; i++) {
            ...
            let range = {}
            range.ideal = CSL.Value.new(
                CSL.BigNum.from_str('0')
            )
                .checked_add(splitOutputsAmounts[i])
                .checked_add(splitOutputsAmounts[i])
            range.maximum = CSL.Value.new(
                CSL.BigNum.from_str('0')
            )
        ...
        if (utxoSelection.remaining.length > 0) {
            ...
            let minAmount = CSL.Value.new(
                CSL.min_ada_required(
                    change,
            ...
            maxFee = CSL.Value.new(
                CSL.BigNum.from_str(maxFee.toString())
            )
            ...
            if (compare(change, minAmount) < 0) {
                ...
                const minAda = minAmount
                    .checked_sub(CSL.Value.new(change.coin()))
                    .checked_add(CSL.Value.new(utxoSelection.amount.coin()))
...
function mergeOutputsAmounts (outputs) {
    let compiledAmountList = CSL.Value.new(
        CSL.BigNum.from_str('0')
    )
...
function splitAmounts (amounts) {
    ...
    if (amounts.multiasset()) {
        ...
        for (let i = 0; i < mA.keys().len(); i++) {
            ...
            for (let j = 0; j < mA.get(scriptHash).keys().len(); j++) {
                let _assets = CSL.Assets.new()
                let assetName = mA.get(scriptHash).keys().get(j)

                _assets.insert(
                    CSL.AssetName.from_bytes(assetName.to_bytes()),
                    CSL.BigNum.from_bytes(
                ...
                _multiasset.insert(
                    CSL.ScriptHash.from_bytes(scriptHash.to_bytes()),
                    _assets
                )
                let _value = CSL.Value.new(
                    CSL.BigNum.from_str('0')
                )
    ...
    splitAmounts.push(
        CSL.Value.new(
            CSL.BigNum.from_bytes(amounts.coin().to_bytes())
        )
    )
...
function isQtyFulfilled (
    ...
    if (minUTxOValue && BigInt(outputAmount.coin().to_str()) > 0) {
        let minAmount = CSL.Value.new(
            CSL.min_ada_required(
                cumulatedAmount,
                CSL.BigNum.from_str(minUTxOValue.toString())
            )
        )
...
const cloneUTxOList = (utxoList) =>
    utxoList.map((utxo) =>
        CSL.TransactionUnspentOutput.from_bytes(utxo.to_bytes())
    )
 ...
 */
const cloneValue = (value) => CSL.Value.from_bytes(value.to_bytes())
